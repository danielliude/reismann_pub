...
def signup(request, signup_form=RegistrationForm,
           template_name='accounts/signup.html', success_url=None,
           extra_context=None):
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      else: redirect_to = userena_settings.USERENA_SIGNIN_REDIRECT_URL % {'username': user.username}
...
def signin(request, auth_form=AuthenticationForm,
           ...
           redirect_signin_function=login_redirect(), extra_context=None):
  """
...
def edit_profile(request, username, edit_profile_form=ProfileForm,
                 template_name='accounts/profile_edit_profile.html', success_url=None,
    ...
    if form.is_valid():
      ...
      if success_url:
        ...
        userena_signals.profile_change.send(sender=None,
                                            user=user)
        redirect_to = success_url
      else: redirect_to = reverse('accounts:profile_detail', kwargs={'username': username})
      return redirect(redirect_to)
...
def edit_contact(request, username, edit_contact_form=ContactForm,
                 template_name='contacts/contact_form.html', success_url=None,
                 extra_context=None, **kwargs):

  user = get_object_or_404(User, username__iexact=username)
  profile = get_user_profile(user=user)
  contact = get_user_contact(user=user)
  ...
  if request.method == 'POST':
    ...
    if form.is_valid():
      ...
      if success_url:
        ...
        redirect_to = reverse('accounts:profile_detail', kwargs={'username': username})
      return redirect(redirect_to)
