...
class ProfileMoreForm(forms.ModelForm):
  ...
  hobby = forms.ChoiceField(choices=HOBBY,
                            ...
                            widget=forms.Select(attrs={'class': 'ui fluid search dropdown'}),  
                            required=False)
  most_cheerful       = forms.CharField(label=_('The most cheerful thing for me'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  good_at             = forms.CharField(label=_('The thing I am good at'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  friends_description = forms.CharField(label=_('3 words which friends uses to describe me'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  must_have           = forms.CharField(label=_('5 things I must have by my side'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  favourite_things    = forms.CharField(label=_('Favourite books, movies and foods'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  crazy_thing_done    = forms.CharField(label=_('Most crazy thing I have done'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)
  normal_do_weekend   = forms.CharField(label=_('What I normally do on weekend'), max_length=50, widget=forms.Textarea(attrs={'rows': "2"},), required=False)

  class Meta:
    model = ProfileMore
    exclude = ['user']
  def save(self, force_insert=False, force_update=False, commit=True):
    profile = super(ProfileMoreForm, self).save(commit=commit)
    return profile
